...
SELECT 
    ...
    (SELECT COUNT(*)
     FROM vehicle_route 
     WHERE vehicle_route.route_id = route.id) AS vehicle_count
FROM route;
...
SELECT 
    ...
    (SELECT vehicle_type.name
     FROM vehicle_type
     WHERE vehicle_type.id = vehicle.vehicle_type_id) AS vehicle_type
FROM
    (SELECT *
     FROM vehicle
     WHERE vehicle.status = 'available') AS vehicle;
...
WHERE 
    EXISTS (
        SELECT 1
        FROM vehicle_driver
        WHERE vehicle_driver.driver_id = driver.id
        AND EXISTS (
            SELECT 1
            FROM vehicle_route
            WHERE vehicle_route.vehicle_id = vehicle_driver.vehicle_id
        )
    );
...
FROM driver
WHERE driver.id IN (
    SELECT vehicle_driver.driver_id
    FROM vehicle_driver
    WHERE vehicle_driver.vehicle_id IN (
        SELECT vehicle.id
        FROM vehicle
        WHERE vehicle.vehicle_type_id IN (
            SELECT vehicle_type.id
            FROM vehicle_type
            WHERE vehicle_type.name = 'Bus'
        )
    )
);
...
JOIN
    vehicle_driver ON vehicle.id = vehicle_driver.vehicle_id
JOIN
    driver ON vehicle_driver.driver_id = driver.id;
...
FROM 
    driver
INNER JOIN
    vehicle_driver ON driver.id = vehicle_driver.driver_id
INNER JOIN
    vehicle ON vehicle_driver.vehicle_id = vehicle.id
