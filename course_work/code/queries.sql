...
WITH officer_counts AS (SELECT police_officers.id                                AS officer_id,
...
SELECT accident_resolutions.id                           AS resolution_id,
...
ORDER BY accident_resolutions.time_of_consideration DESC;
...
WHERE violations.time_of_violation >= CURRENT_DATE - INTERVAL '1 year'
GROUP BY citizens.id
ORDER BY COUNT(violations.id) DESC
LIMIT 10;

-- знайти кількість порушників та порушень, які скоєні на автомобілях із звичайними номерами та іменними
WITH categorized_vehicles AS (SELECT vehicles.id,
                                     vehicles.registration_number,
                                     get_plate_type(vehicles.registration_number) AS plate_type
                              FROM vehicles),
     violations_summary AS (SELECT categorized_vehicles.plate_type,
                            ...
                            WHERE categorized_vehicles.plate_type IN ('regular', 'personalized')
                            GROUP BY categorized_vehicles.plate_type)
SELECT plate_type,
       violation_count
...
SELECT full_vehicle_info.vehicle_id,
       ...
       COUNT(violations.id) AS violation_count
FROM full_vehicle_info
         JOIN violations ON full_vehicle_info.vehicle_id = violations.vehicle_id
WHERE violations.time_of_violation >= CURRENT_DATE - INTERVAL '1 month'
GROUP BY full_vehicle_info.vehicle_id,
         full_vehicle_info.registration_number,
         full_vehicle_info.model,
         full_vehicle_info.brand,
         full_vehicle_info.color,
...
WHERE administrative_offenses.penalty_fee > 0
ORDER BY accident_resolutions.time_of_consideration DESC;

-- список адміністративних порушень із кількістю порушень
SELECT administrative_offenses.id AS offense_id,
...
GROUP BY administrative_offenses.id,
...
FROM evidences
ORDER BY evidences.violation_id,
         evidences.id;
...
FROM drivers
         ...
         JOIN administrative_offenses ON violations.administrative_offense_id = administrative_offenses.id
WHERE violations.time_of_violation BETWEEN drivers.license_issued_time
...
SELECT vehicles.brand,
       ...
       (COUNT(violations.id) :: DECIMAL / COUNT(vehicles.id) * 100) AS violation_percentage
FROM vehicles
         LEFT JOIN violations ON vehicles.id = violations.vehicle_id
...
WHERE citizens_on_protocol.role = 'witness'
GROUP BY citizens.id,
         citizens.first_name,
         citizens.last_name,
         citizens.patronymic
...
SELECT get_citizen_full_name(citizens.id)                    AS full_name,
...
SELECT vehicle_types.name                                          AS vehicle_type,
...
SELECT accident_protocols.id AS protocol_id,
       COUNT(
               CASE
                   WHEN citizens_on_protocol.role = 'witness' THEN 1
                   END
...
GROUP BY accident_protocols.id
ORDER BY accident_protocols.id;
...
FROM citizens
         JOIN vehicles ON citizens.id = vehicles.owner_id
GROUP BY citizens.id
HAVING COUNT(DISTINCT vehicles.vehicle_type_id) > 1
ORDER BY vehicle_types DESC;


WITH ranked_officers AS (SELECT police_officers.rank,
                                police_officers.id             AS officer_id,
                                COUNT(accident_protocols.id)   AS protocol_count,
                                COUNT(accident_resolutions.id) AS resolution_count,
                                RANK() OVER (
                                    PARTITION BY police_officers.rank
                                    ORDER BY
                                        COUNT(accident_protocols.id) DESC
                                    )                          AS rank
                         FROM police_officers
                                  LEFT JOIN accident_protocols
                                            ON police_officers.id = accident_protocols.police_officer_id
                                  LEFT JOIN accident_resolutions
                                            ON police_officers.id = accident_resolutions.police_officer_id
                         GROUP BY police_officers.rank,
                                  police_officers.id)
SELECT police_officers.rank,
       police_officers.badge_number,
       ranked_officers.protocol_count,
       ranked_officers.resolution_count
FROM police_officers
         JOIN ranked_officers ON police_officers.id = ranked_officers.officer_id;